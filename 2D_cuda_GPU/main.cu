...
__global__ int FFT2D_Radix2_DIT_kernel_row(complexFloat *data, int height, int width) {

    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < height) {
        complexFloat *row = (complexFloat *)alloca(width * sizeof(complexFloat));
...
___global___ int FFT2D_Radix2_DIT_kernel_column(complexFloat *data, int height, int width) {
    int j = blockIdx.x * blockDim.x + threadIdx.x;
    if (j < width) {
        complexFloat *col = (complexFloat *)alloca(height * sizeof(complexFloat));
...
___global___ int FFT2D_Radix2_DIT(complexFloat *data, int height, int width) {
    // ----------- Row-wise FFT ------------
    int blockSize = 256;
    int numBlocks = (height + blockSize - 1) / blockSize;
    FFT2D_Radix2_DIT_kernel_row<<<numBlocks, blockSize>>>(data, height, width);
    cudaDeviceSynchronize();

    // ----------- Column-wise FFT ------------
    numBlocks = (width + blockSize - 1) / blockSize;
    FFT2D_Radix2_DIT_kernel_column<<<numBlocks, blockSize>>>(data, height, width);
    cudaDeviceSynchronize();
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    //Allocate memory for FFT input, spectrum, and IFFT output
